...
function withFrameworkConfig(defaultConfig = {}) {
  ...
  if (!framework) {
    throw new Error("Framework Api is missing, please add a valid provider");
  }

  if (!ALLOWED_FW.includes(framework)) {
    throw new Error(
      `The API framework: ${freamwork} connot be found, please use of the following: ${ALLOWED_FW.join(
        ", "
      )}`
    );
  }
